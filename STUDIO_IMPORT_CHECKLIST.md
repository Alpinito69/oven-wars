...
|------:|-------------|------------------|----------|
| 1 | Plot `Oven` | Tiny Treats Charming Kitchen `stove.fbx` (CC0) / Bakery Interior oven | **`BasePart` named `Oven`** (MeshPart OK; not Model-only) + `IsOven`; keep `BakeLight`, `SteamAttachment`. Required by ProximityGuard. |
...
| 5 | Hub stalls | Market Stall `5626984031`, Beverage Market Stall `17641160`, Shop Stand `128117050019864` | Prompt-host BasePart: `IsGatherNode` + **`GatherNodeId`** + `GatherItemId` |
...
- `Oven` **BasePart** (MeshPart OK) + `IsOven` + optional `BakeLight` + `SteamAttachment` — ProximityGuard requires BasePart, not Model-only
- `DisplayCase` + `StealPrompt` + `StealAttachment`
- `Counter` + `SellAttachment`
- `UpgradeBoard` with `IsUpgradeBoard`
- `Spawn` marker part
- Hub gather hosts: `IsGatherNode` + **`GatherNodeId`** + `GatherItemId` on prompt-host BasePart
